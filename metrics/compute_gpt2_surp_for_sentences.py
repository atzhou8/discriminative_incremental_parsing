...
from utils import split_sentence_by_word
...
def compute_word_surprisals_for_sentence(sentence, tokenizer, model, device):
    ...
    input_ids = torch.cat(
        [torch.full((1, 1), bos_id, dtype=inputs['input_ids'].dtype), inputs['input_ids']],
        dim=1
    )
    attention_mask = torch.cat(
        [torch.ones((1, 1), dtype=inputs['attention_mask'].dtype), inputs['attention_mask']],
        dim=1
    )
    ...
    for i, (token, surp) in enumerate(zip(tokens[1:], token_surprisals)):
        # GPT-2 has no <s>/<pad>; <|endoftext|> serves as both bos and eos
        if token in ['<|endoftext|>']:
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output_csv', default='out/gpt2_word_surprisals.csv')
    parser.add_argument('-m', '--model_name_or_path', default='gpt2',
                         help="HF model id or local path, e.g. 'gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl'")
