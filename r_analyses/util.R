...
Predicting_RT_with_spillover <- function(rt.data_df,subsetname, models = c('gpt2', 'lstm',"nosurp"), parser_predictor_col='kl_forward'){
  ...
  if(subsetname=="Agreement"){
    ...
    for(model in models){
      if(i!=3){
        ...
        if (startsWith(model, "parser")) {
          surps <- read.csv(paste0('../out/sud_with_adjunct/items_', subsetname, '.parser.csv.scaled'))
          surps$surprisal <- as.numeric(surps[[parser_predictor_col]])
          ...
          distinct()
          surps <- surps %>%
          left_join(lstm_ref, by=c("Sentence", "word_pos")) %>%
          select(Sentence, word_pos, surprisal, surprisal_s, logfreq, logfreq_s, length, length_s)
        } else{ 
          ## NOTE THIS ASSUMES THE FOLDER NAME FOR LSTM MODELS IS lstm  AND NOT gulordava
          surps <- read.csv(paste0('../../Surprisals/data/', model,'/items_',subsetname,'.', model, '.csv.scaled')) %>%
            mutate(word_pos = word_pos + 1,
        ...
        filler.model <- readRDS(paste0('./filler_models/filler_', model, '.rds')) 
